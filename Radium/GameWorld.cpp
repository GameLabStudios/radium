...
void GameWorld::buildScene()
{
    ...
    // Add a basic shooting enemy to scene
    Vector2f shootSpawnPos = mSpawnPosition + Vector2f(100.0f, 100.0f);
    std::unique_ptr<ShooterEnemy> shootingEnemy(new ShooterEnemy(shootSpawnPos));
    mShooterEnemy = shootingEnemy.get();
    shootingEnemy->setPosition(shootSpawnPos);
    mSceneLayers[Background]->attachChild(std::move(shootingEnemy));
